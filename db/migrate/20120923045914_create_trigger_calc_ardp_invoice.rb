class CreateTriggerCalcArdpInvoice < ActiveRecord::Migration
  def up
    execute <<-SQL

      create or replace
      TRIGGER calc_ar_dp_invoice
      AFTER INSERT OR UPDATE ON AR_DP_INVOICE_ITEMS
      FOR EACH ROW
      DECLARE
      ...
      BEGIN
        IF :old.id IS NULL THEN
          do_id := :new.ar_dp_invoice_id;
        ELSE
          do_id := :old.ar_dp_invoice_id;
        END IF;
  ...
  def down
    execute "DROP TRIGGER calc_ar_dp_invoice"
  end
end
