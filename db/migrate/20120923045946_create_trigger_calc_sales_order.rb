class CreateTriggerCalcSalesOrder < ActiveRecord::Migration
  def up
    execute <<-SQL

      create or replace
      TRIGGER calc_sales_order
      ...
      BEGIN
        IF :old.id IS NULL THEN
          do_id := :new.sales_order_id;
        ELSE
          do_id := :old.sales_order_id;
        END IF;
  ...
  def down
    execute "DROP TRIGGER calc_sales_order"
  end
end
