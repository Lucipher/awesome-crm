ActiveAdmin.register ArInvoice, :as => "AR Invoice", :namespace => false do
  menu :parent => "Forms"
  ...
  show do |r|
    ...
    div :class => "panel" do
      h3 "Item List"
      if r.ar_invoice_items and r.ar_invoice_items.count > 0
  ...
  form do |f|
    ...
    f.inputs "Item List" do
      f.has_many :ar_invoice_items do |fi|
        fi.input :item
        fi.input :line_num
  ...
  controller do
    def create
      @ar_invoice = ArInvoice.new(params[:ar_invoice])
      @ar_invoice.id = ArInvoice.connection.select_value("SELECT crm.ar_invoices_seq.nextval FROM DUAL")
      if @ar_invoice.save
        redirect_to @ar_invoice, :notice => "AR invoice was successfully created"
      else
        render :new
      end
    end
